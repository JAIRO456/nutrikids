        ...
        if ($sqlInsertDirector->execute([$documento, $nombre, $apellido, $email, $telefono, $password, $fileName, 2, 2])) {
            ...
            if ($sqlInsertDetails->execute([$documento, $id_escuela])) {
                ...
                if (email_password($email['email'], $email['nombre'], $email['apellido'], $email['documento'], $contrasena_original)) {
                    echo "<script>
                            document.addEventListener('DOMContentLoaded', function() {
                                showModal('El director se ha creado exitosamente, se le ha enviado un correo de notificación.', 'success');
                                setTimeout(() => {
                    ...
                    echo "<script>
                            document.addEventListener('DOMContentLoaded', function() {
                                showModal('El director se ha creado exitosamente, pero hubo un error al enviar el correo.', 'error');
                                setTimeout(() => {
                                    window.location = '../directores.php';
                                }, 3000);
                            });
                ...
                echo "<script>
                        document.addEventListener('DOMContentLoaded', function() {
                            showModal('Error al asignar la escuela al director.', 'error');
                        });
                    </script>";
            }
        } 
            ...
            echo "<script>
                    document.addEventListener('DOMContentLoaded', function() {
                        showModal('Error al registrar al director.', 'error');
                    });
                </script>";
        }
    }
