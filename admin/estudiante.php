...
<?php
    if (isset($_POST['submit-button'])){
        $name_est = $_POST['nombre_est'];
        $ape_est = $_POST['apellido_est'];
        $doc_est = $_POST['documento_est'];
        $email_est = $_POST['email_est'];
        $tel_est = $_POST['telefono_est'];
        $esc_est = $_POST['escuela_est'];
        
        $doc = $_POST['documento'];

        $sqlUser = $con -> prepare("SELECT * FROM familias WHERE documento_est = $doc_est");
        $sqlUser -> execute();
        ...
        if ($doc_est == "" || $name_est == "" || $ape_est == "" || $email_est == "" || $tel_est == "" || $esc_est == ""|| $estado == ""){
            echo "<script>alert('Existen datos vacios del estudiante')</script>";
            echo "<script>window.location = 'acudiente.php'</script>";
        }

        else{
            $sqlInsert = $con->prepare("INSERT INTO estudiantes (documento_est, nombre_est, apellido_est, email_est, telefono_est, id_escuela, id_estado) VALUES ('$doc_est', '$name_est', '$ape_est', '$email_est', '$tel_est', '$esc_est', '$estado')");
            $sqlInsert->execute();
            $sqlInsert2 = $con->prepare("INSERT INTO familias (documento, documento_est) VALUES ('$doc', '$doc_est')");
            $sqlInsert2->execute();
